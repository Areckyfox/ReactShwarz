...
class App extends Component {
  state = {
    person: [{ userName: "Geralt", userFrom: "Rivia" }]
  };

  changeNameHandler = (event) => {
    this.setState({
      person: [{ userName: event.target.value, userFrom: "Rivia" }]
    });
  }
  render() {
    return (
      <div>
        <UserInput
          currentName={this.state.person[0].userName}
          changed={this.changeNameHandler}
          value = {this.state.person[0].userName}
        />
